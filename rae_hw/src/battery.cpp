...
namespace rae_hw
{
  ...
  void Battery::logStatus(const sensor_msgs::msg::BatteryState &message)
  {
    ...
    {
      RCLCPP_INFO(this->get_logger(), "Power supply status changed to [%s] after %d h %d min %d secs.",
                  stateMsgToString.at(message.power_supply_status).c_str(),
                  int(stateHours), int(stateMins % 60), int(stateSecs % 60));
    ...
    if (message.capacity < 100.0 && message.capacity != prevState.capacity && (int)message.capacity % 10 == 0)
    {
      ...
      RCLCPP_INFO(this->get_logger(), "Battery capacity: %f, Status: [%s] for %d h %d min %d s. Time since last log: %d h %d min %d secs.",
                  message.capacity, stateMsgToString.at(message.power_supply_status).c_str(),
                  int(stateHours), int(stateMins % 60), int(stateSecs % 60),
                  int(logHours), int(logMins % 60), int(logSecs % 60));
