...
SpeakersNode::SpeakersNode(const rclcpp::NodeOptions & options) : Node("speakers_node", options)
{
  ...
  play_audio_service_ = create_service<rae_msgs::srv::PlayAudio>(
    "play_audio", std::bind(
                    &SpeakersNode::play_audio_service_callback, this, std::placeholders::_1,
...
void SpeakersNode::play_mp3(const char * mp3_file)
{
  ...
  if (
    mpg123_open(mh, mp3_file) != MPG123_OK ||
  ...
  snd_pcm_set_params(
    alsaHandle, SND_PCM_FORMAT_S16_LE, SND_PCM_ACCESS_RW_INTERLEAVED, channels, rate * channels, 2,
    50000);
