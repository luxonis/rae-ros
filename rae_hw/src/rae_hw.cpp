...
hardware_interface::CallbackReturn RaeHW::on_init(const hardware_interface::HardwareInfo & info)
{
  ...
  PID pidL{
    std::stof(info_.hardware_parameters["PID_P_L"]),
    std::stof(info_.hardware_parameters["PID_I_L"]),
    std::stof(info_.hardware_parameters["PID_D_L"])};
  motorL = std::make_unique<RaeMotor>(
    leftWheelName, chipName, pwmName, pwmL, phL, enLA, enLB, encTicsPerRevL, maxVelL, true,
    closedLoopL, pidL);
  ...
  PID pidR{
    std::stof(info_.hardware_parameters["PID_P_R"]),
    std::stof(info_.hardware_parameters["PID_I_R"]),
    std::stof(info_.hardware_parameters["PID_D_R"])};
  motorR = std::make_unique<RaeMotor>(
    rightWheelName, chipName, pwmName, pwmR, phR, enRA, enRB, encTicsPerRevR, maxVelR, false,
    closedLoopR, pidR);
