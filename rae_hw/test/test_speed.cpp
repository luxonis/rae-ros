...
int main(int argc, char * argv[])
{
  ...
  if (argc == 2 && std::strcmp(argv[1], "-h") == 0) {
    ...
    std::cout
      << "ros2 run rae_hw test_encoders duration encRatioL encRatioR speedL speedR P I D.\n";
    std::cout
      << "With default arguments ros2 run rae_hw test_encoders 5.0 187 187 16.0 16.0 0.5 0.0 0.0"
      << std::endl;
    return 0;
  }
  if (argc > 1 && argc < 9) {
  ...
  auto motorL = std::make_unique<rae_hw::RaeMotor>(
    "left_wheel_name", "gpiochip0", "/sys/class/pwm/pwmchip0", 2, 41, 42, 43, encRatioL, 32, true,
    true, pid);
  auto motorR = std::make_unique<rae_hw::RaeMotor>(
    "right_wheel_name", "gpiochip0", "/sys/class/pwm/pwmchip0", 1, 45, 46, 47, encRatioR, 32, false,
    true, pid);
  motorL->run();
  motorR->run();
  float leftPos = 0.0;
