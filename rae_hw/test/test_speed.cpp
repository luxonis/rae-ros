...
int main(int argc, char* argv[]) {
    ...
    auto motorL = std::make_unique<rae_hw::RaeMotor>("left_wheel_name", "gpiochip0", "/sys/class/pwm/pwmchip0", 2, 41, 42, 43, encRatioL, 32, true, true, pid);
    auto motorR =
        std::make_unique<rae_hw::RaeMotor>("right_wheel_name", "gpiochip0", "/sys/class/pwm/pwmchip0", 1, 45, 46, 47, encRatioR, 32, false, true, pid);
    motorL->run();
    motorR->run();
    float leftPos = 0.0;
    float rightPos = 0.0;
