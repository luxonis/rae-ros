...
int main(int argc, char * argv[])
{
  ...
  if (argc == 2 && std::strcmp(argv[1], "-h") == 0) {
    ...
    std::cout << "ros2 run rae_hw test_motors duration speedL speedR encRatioL encRatioR maxVelL "
                 "maxVelR.\n";
    std::cout << "With default arguments ros2 run rae_hw test_motors 5.0 16.0 16.0 756 756 32 32"
              << std::endl;
    return 0;
  }
  if (argc > 1 && argc < 8) {
  ...
  auto motorL = std::make_unique<rae_hw::RaeMotor>(
    "left_wheel_name", "gpiochip0", "/sys/class/pwm/pwmchip0", 1, 41, 42, 43, encRatioL, maxVelL,
    true);
  auto motorR = std::make_unique<rae_hw::RaeMotor>(
    "right_wheel_name", "gpiochip0", "/sys/class/pwm/pwmchip0", 2, 45, 46, 47, encRatioR, maxVelR,
    false);
