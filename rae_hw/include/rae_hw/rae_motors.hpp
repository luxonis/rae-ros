...
struct State
{
  int A;
  int B;
  bool operator==(const State & rhs) { return (A == rhs.A && B == rhs.B); }
  bool operator!=(const State & rhs) { return (A != rhs.A || B != rhs.B); }
};
struct PID
{
...
public:
  RaeMotor(
    const std::string & name, const std::string & chipName, const std::string & pwmName,
    int pwmPinNum, int phPinNum, int enA, int enB, float encTicsPerRev, float maxVel,
    bool reversePhPinLogic, bool closedLoop = false, PID pid = {0.6, 0.4, 0.0});
  ~RaeMotor();
